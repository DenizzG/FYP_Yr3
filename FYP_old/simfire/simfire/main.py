...
from simfire.render_env import RenderEnv
...
def train(algo: Algorithm, cfg: DictConfig) -> None:
    ...
    for i in range(stop_cond.training_iteration):
        ...
        if i % cfg.checkpoint.checkpoint_frequency == 0:
            ...
            # deleted code, check github 
            
            with tempfile.TemporaryDirectory() as temp_checkpoint_dir:
                
                checkpoint_dir = os.path.join(root_checkpoint_dir, f"checkpoint_{i}")

                # Save your model or algorithm state to the temporary directory
                algo.save(checkpoint_dir=temp_checkpoint_dir)
                
                # Create a Checkpoint object from the directory
                checkpoint = Checkpoint.from_directory(temp_checkpoint_dir)
                
                # Report the checkpoint (optional: include metrics)
                train.report({"iteration": i}, checkpoint=checkpoint)
                
                # Log the checkpoint path
                LOGGER.info(
                    f"An Algorithm checkpoint has been created at iteration {i}."
                )
