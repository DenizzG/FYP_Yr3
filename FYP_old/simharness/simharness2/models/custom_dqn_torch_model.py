...
class CustomDQNTorchVisionNet(VisionNetwork):
    ...
    def metrics(self) -> Dict[str, TensorType]:
        ...
        if hasattr(self, "tower_stats") and "q_loss" in self.tower_stats: #tower_stats is where RLlib stores immediate stats during the training loop.
            q_loss: QLoss = self.tower_stats["q_loss"] #check - does tower stats exist and does it contain q_loss
            if q_loss.loss.device == "cpu":
                loss = q_loss.loss.detach().item()
            else:
                loss = q_loss.loss.detach().cpu().item()
            return {"q_loss": loss}
        else: ##fallback if no q loss is available 
            return {}
